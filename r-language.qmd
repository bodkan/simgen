...
When we create a normal plain R list...

```{r}
homebrew <- list(col_a = 1:3, col_b = c("one", "two", "three"))
homebrew
```

... and assing a "data.frame" class attribute to it like this...

```{r}
class(homebrew) <- "data.frame"
```
