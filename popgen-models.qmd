...
programmable modeling interface of the R package _slendr_. Admittedly, there
are many valid definitions for what a "demographic model" could be, but in the
context of this book, we will generally think about models as "tree-like
...
**Hint:** If you ever get _really_ stuck, you can always take a look at
solutions provided with each exercise. The point of these exercises is to learn
things and get you comfortable with computer programming, not fight through
endless frustration. That said, a little frustration is a necessary component
of any learning process, so try not to look at the solutions automatically and
...
containing the following "template script". Then add a sequence of appropriate
...
What do you think happens when you do that? Take a look at `?msprime` if you
need a hint.**
...
be long, so you might have to scroll horizontally in your browser):

```{r}
msprime(model, sequence_length = 1e6, recombination_rate = 1e-8, run = FALSE)
```

Similarly to the symbol `model`, whenever we from now on refer to `ts`, we will be talking about some simulated tree-sequence object stored in an R variable `ts`.
...
table-based implementation (much more information on that is
...
**Then, schedule the sampling of a single `eur` individual at each of the the
...
**In total, this should give you a data frame containing the sampling schedule
for the recording of of the genomes of 38 simulated individuals.**
...
**Note:** As you've seen above, the visualization is often a bit
wonky and convoluted with overlapping elements and it can be even worse with
samples added, but try to experiment with arguments to `plot_model` described
above to make the plot a bit more helpful for sanity checking.
...
**Use your combined sampling `schedule` and run a new tree-sequence simulation
