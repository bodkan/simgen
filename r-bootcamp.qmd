...
Before we even get started, there's one thing we should make clear: R (and
...
discussed in the first Exercise) will be _coreced_ to be only one of those types,
given certain rules.
...
**Out of all these data type exploration, this Exercise is probably the most crucial
...
penguin_hist <- function(df, column, breaks) {
  if (breaks < 1) {
    stop("Incorrect breaks argument given!")
  }
...
These are perhaps too boring to see immediate usefulness of `sapply()` and
`lapply()` for real-world applications. In later sessions, we will see more
