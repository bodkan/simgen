...
Create a vector of numbers 7:23 like this.**
...
shortcut above to get values `7-23`.**
...
**`seq()` is one of the most useful functions, so keep it in mind!**
...
or `!B`? How do these logical operators work when you have multiple values?
Can you deduce this from the results of these `&`, `|`| and `!` expressions
on `A` and `B` vectors?**
...
**Hint:** Remember that evaluating `coverage > 5` gives you a logical vector
...
**Hint:** Apply the `all()` function to a logical expression you get by
comparing both variables you just created with the `==` operator.
...
Here are some more advanced materials. These are very useful to know (and
actually critical for writing more complex programs), but are not completely
necessary to be able to successfuly go through the later sessions on
_tidyverse_ and _ggplot2_. Feel free to go through them for your own interest
