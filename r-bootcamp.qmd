...
```{r}
estonian <- c(FALSE, TRUE, FALSE, FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, 
FALSE)
```

**Write now a more complex conditional expression, which will test if a
given individual has (again) coverage higher than 5 and is "modern" OR
whether it's Estonian (and so it's coverage or "modern" state doesn't
matter).**
...
```{r}
(coverage > 5 & origin == "modern") | estonian
```
