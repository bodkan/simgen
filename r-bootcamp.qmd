...
from above and experiment with both versions of logical AND and OR operators.**
...
representing a position in the vector/list),
...
how to do something immediately in your head---I never do, at least. I always
...
We can then ask, for instance, _"which elements of `v` are among a set
of given values?"_:
...
**Hint:** Create a variable with the list with the same vectors as are in the
...
which you're indexing with a comma in this way: `[row-index, 
column-name-or-index]`.**

---

**Extract the first three elements (`1:3`) of the data frame `df` by
`df[1:3, ]`.**

**Note:** Notice the empty space after the comma `,` in the question!
...
**Then select a subset of the `df` data frame to only show the row #1 and #4
for columns `"x"` and `"z"`.**
...
of the `TRUE` / `FALSE` column `z` into integers 1 or 0. Use `as.integer()`
to do this.**
...
is not automated and, as a result, it's not reprodubile.
...
of _looping_ introduced in one of the "Bonus exercises" towards the end of
this Bootcamp session.
...
necessary to be able to successfully go through the later sessions on
_tidyverse_ and _ggplot2_.

Feel free to go through them for your own interest though! They are very useful.
Just don't be stressed if this seems too much (especially if you haven't
done a lot of R coding before).
