...
pipeline which is the final product from second session on _tidyverse_. Create a new R script in RStudio
(`File` `->` `New file` `->` `R Script`), and save it somewhere on your
computer as `tidy-viz.R` (`File` `->` `Save`). Copy the following bit of code
which will execute an IBD and metadata processing pipeline into that script.**

The following exercises will focus on basics of visualization of various
types of statistical summaries, using our IBD data set as a representative
...
making _ggplot2_ figures. 🙂 Everything else, all the amazing complexity
