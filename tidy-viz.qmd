...
variable `metadata_subset`: `sampleId`, `popId`, `region`, `ageAverage`, `flag`.
Then `rename()` `sampleId` to `sample`, `popId` to `pop`, and `ageAverage` to
`age` just to save ourself some typing.**

::: {.callout-note collapse="true" icon=false}
#### Click to see the solution

```{r}
metadata <- select(metadata_all, sample = sampleId, pop = popId, region, age = ageAverage, set = groupAge)
```
...
`NA` (and everyone else's age stays the same).

::: {.callout-note collapse="true" icon=false}
#### Click to see the solution

```{r}
metadata <- mutate(metadata, age = if_else(set == "Modern", 0, age))
```
...
```{r}
metadata <- filter(metadata, set != "Archaic")
```
...
## Exercise 4: Metadata annotation
...
as a time series. **Let's introduce an incredibly useful function called `cut()`.
Take a look at `?cut` help page and skim through it to figure out what it does.
As a bit of a hint, we now want to add a new metadata columns which will indicate
in which age bin (maybe, split in steps of 5000 years) do our individuals
belong to.**

**Hint:** Here's a small example to help you get started. Think about what
the `cut()` function does here based on the result it gives you here (and,
again, don't forget to look at optional arguments you can read about in `?cut`).
...
df$bin <- cut(df$age, breaks = seq(0, 50000, by = 5000))

df
```

Or, in a _tidyverse_ context:

```{r}
df <- tibble(age = c(0, 0, 1000, 0, 5000, 10000, 7000, 13000, 18000, 21000, 27000, 30000))
df <- mutate(df, bin = cut(age, breaks = seq(0, 50000, by = 5000)))
...
**Hint:** Whenever you have many potential categories like here, `seq()` is
very useful to generate custom breakpoints. We wouldn't want to write out
the boundaries manually! Again, don't forget you can read `?seq` and experiment
in R console with little bits of code before you commit to working on your full
data! This makes things much more manageable -- figure out how things work on
a small toy data is the best way to do anything.

---

**In the example of the `cut()` function right above, what is the data type
of the column `bin` created by the `cut()` function? Use `glimpse(df)` to see
this data type, then skim through the documentation of `?factor`. Additionally,
why is the `bin` value equal to `NA` for some of the values? (We will fix this
later, don't worry.)**
...
```{r}
bin_levels <- levels(df$bin) # 1. extract bin labels (no "present-day" category yet)

df <-
  df %>%
  mutate(
    bin = as.character(bin),                      # 2. convert factors into strings
    bin = if_else(is.na(bin), "present-day", bin) # 3. replace NA with "present-day"
  )
```
...
2000 years again. Use the example from above to do this.**
...
## Exercise 4: Investigating IBD data
...
```{r}
ibd <- inner_join(ibd_orig, metadata1)
