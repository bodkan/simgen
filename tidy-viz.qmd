...
ibd_orig <- read_tsv(gz_file, show_col_types = FALSE)
```
...
```{r}
ibd_samples <- unique(c(ibd_orig$sample1, ibd_orig$sample2))
...
to a new variable `ibd_all`.**

::: {.callout-note collapse="true" icon=false}
#### Click to see the solution

```{r}
ibd_all <- ibd_orig %>% mutate(length = end - start)
...
ibd_all$length <- ibd_orig$end - ibd_orig$start
...
```{r}
ibd_all <- inner_join(ibd_all, metadata1)
ibd_all <- inner_join(ibd_all, metadata2)
...
```{r}
ibd_all <- mutate(ibd_all, time_pair = paste(age_bin1, age_bin2, sep = ":"), .after = region_pair)
...
to our huge `ibd_all` table, to achieve the following:**

0. Start with our original big IBD table `ibd_all` (not the smaller `ibd_eur`!)
...
of individuals),
6. `ungroup()` at the end to remove the grouping aspect.
...
ibd_sum <-
  ibd_all %>%
  filter(region_pair == "Europe:Europe" & length > 10 & age_bin1 == age_bin2) %>% 
  group_by(sample1, sample2, country_pair, time_pair) %>%
...
Obviously, because scatter plot is two-dimensional, we have to specify
not only the `x` but also the `y` in the `aes()` aesthetic mapping... and with
that, I think you already know how to do this! :) **Use the function
`geom_point()` to visualize the scatter plot of `x = total_ibd` against
`y = n_ibd`. In this educational exercise, we have of course IBD data for only
a small portion of genome (due to memory restrictions). But still, do you
see similarities to a figure from [this huge](https://www.nature.com/articles/s41588-023-01582-w/figures/3)
study of IBD patterns and their relationship to the degree of relatedness
between individuals? Which pairs of individuals (remember, each dot in your
scatter plot is a pair of people) could be related, perhaps even first-degree
relatives?**
