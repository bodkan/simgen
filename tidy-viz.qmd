...
```
ggplot() +
  geom_point(data = ibd_unrel, aes(x = n_ibd, y = total_ibd), color = "lightgray", size = 0.75)
```
...
this: `+ geom_line(stat = "smooth", se = FALSE)`. What happens when you run
this modified version of your code? Read `?geom_line` to figure out what `se = FALSE` means.**
