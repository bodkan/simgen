...
ibd_all <- join_metadata(ibd_segments, metadata)
```
...
```{r}
ibd_sum_long <- read_tsv("https://tinyurl.com/simgen-ibd-long", show_col_types = FALSE)
ibd_sum_long <- read_tsv("files/tidy/ibd_long.tsv", show_col_types = FALSE)
```
...
**Take your `ibd_sum_total` table and create two versions of it using `filter()`
like this:**

1. `ibd_unrel <- filter(ibd_sum, is.na(rel))`
2. `ibd_rel <- filter(ibd_sum, !is.na(rel))`
...
```{r}
ibd_unrel <- filter(ibd_sum_long, is.na(rel))
ibd_unrel
```

```{r}
ibd_rel <- filter(ibd_sum_long, !is.na(rel))
ibd_rel
```
...
ggplot() +
  ...
  theme_minimal() +
  facet_grid(~ time_pair)     # <-- THIS IS THE ONLY THING WE ADDED!
```
...
**It's time for our final exercise. I'd like you to read two tables:**

1. Table of counts and total IBD lengths based on IBD segments longer than 10 cM
(you already worked with this above):

```{r}
ibd_sum_long <- read_tsv("https://tinyurl.com/simgen-ibd-long")
```

2. Table of the same information, but based on IBD segments longer than 3 cM (so
summary statistics based on segments much shorter):

```{r}
ibd_sum_short <- read_tsv("https://tinyurl.com/simgen-ibd-short")
```

**Convince yourself that they contain the expected information using something
like `head()`, to verify that both have the same columns.**


```{r}
#| echo: false
plot_ibd <- function(ibd, x_limit = NULL, y_limit = NULL) {
  ibd_unrel <- filter(ibd, is.na(rel))
  ibd_rel <- filter(ibd, !is.na(rel))
...
```{r}
plot_grid(
  plot_ibd(ibd_sum_short, y_limit = c(0, 250)),
  plot_ibd(ibd_sum_long, y_limit = c(0, 250))
)
```
