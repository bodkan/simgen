...
init_env(uv = TRUE) # This activates the internal Python environmet
```
...
```{r}
gf <- gene_flow(from = pop1, to = pop2, start = 2000, end = 2200, proportion = 0.13)
```

::: {.aside}
**Note:** Here `proportion = 0.13` means 13% migrants over the given time window will come from "pop1" into "pop2".
:::
...
#| eval: false
gf <- list(
  gene_flow(from = pop1, to = pop2, start = 500, end = 600, proportion = 0.13),
  gene_flow(from = ..., to = ..., start = ..., end = ..., proportion = ...),
  ...
)
```
...
1. Go to [bodkan.net/simgen](https://bodkan.net/simgen)

2. Start with the section on _"Demographic models"_

3. Then, continue with the section on _"Tree-sequence statistics"_

For convenience, you can refer to information from these slides using their
single-page version under the _"Resources"_ section.
...
<h3>Don't rush and don't panic!</h3>
The exercises are designed so that you all take home information useful
for your current skills and knowledge.
