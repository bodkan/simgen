...
of tabular data. Many operations that were annoying or cumbersome to do
during the R Bootcamp session are easy and natural to do with _tidyverse_
packages like _dplyr_.

2. [_readr_](https://readr.tidyverse.org) provides very functions for reading
...
with these three lines of code, which load all functions from these two packages.**
...
([reference](https://www.nature.com/articles/s41586-023-06865-0)). **Please
take a couple of minutes to skim the paper to get an idea about the sort
of data we will be using during our exercises.**

If you work with anything related to human
history, you'll probably become closely familiar with this study at some point.
...
and store the loaded table in a variable `df`.** Everything in the exercises in
this session will
...
this book at hand every time I work with R.
...
**What is the source of the sequences in our data set (i.e., which are the
...
**Always keep an eye on strange outliers, missing data `NA`, anything
...
obtain from third-party inference software.
...
kinds of variables/columns we have for every individual. It's time to learn
about techniques for _manipulating_, _modifying_, and _filtering_ this data.**
...
We can select which columns to pick from a (potentially very large data frame)
with the function `select()`. It has the following general format:
...
**Note the most important thing for you to notice, which is the absence of "double
...
that you can assign a result to a variable using the `<-` operator):**
...
If your tables has many columns of interest, typing them all by hand like you
did in the `select()` examples above can become tiresome real quick.
