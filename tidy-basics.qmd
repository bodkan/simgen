...
The command below will read a metadata table with information about
...
and store the metadata table in a variable `df` (it's a short name
and we'll be typing it _a lot_ in this session).**
...
the `library()` calls and the `read_tsv()` command above in this script.**

**Unlike the previous R Bootcamp session in which it didn't really matter
if you wrote in a script or R console, in this session, whenever you're done
experimenting in the R console, always record your solution in your R
script. I recommend separating code for different exercises with comments,
...
data frames (they behave the same anyway).
...
(you should decide which function is appropriate for which question).**

Before you use one of these functions for the first time, take a moment to
skim through its help using the `?FUNCTIONNAME` command,
just to build that habit of not forgetting that the help pages are always there.


---

**What variables (i.e., columns) do we have in our data? Think about
what could they mean (some will be obvious, some less so, as is always
the case with unknown data from a supplementary material of a paper). What do the
...
**What are the columns we have that describe the ages (maybe look at those
which have "age" in their name you detected with the `columns()` function earlier)?
How many missing values (`NA`) are in the `ageAverage` column? What information does the `groupAge` column probably
contain?**

**Hint:** Remember that for a column vector `df$COLUMN_NAME`, the command `is.na(df$COLUMN_NAME)`
gives you the value `TRUE` for each `NA` element in that column variable, and
`sum(is.na(df$COLUMN_NAME))` then counts the number of those `NA` values (because
`TRUE` counts as 1, `FALSE` as 0). Alternatively,
`mean(is.na(df$COLUMN_NAME))` counts the proportion of these `NA` values
(because it computes the proportion of 1s).
...
It looks like the `ageAverage` variable has the proportion
...
How many "modern" (i.e., present-day humans) individuals (column `groupAge`)?**
...
<!-- all come from a single study? The answer is in the column `dataSource`, -->
<!-- and you can use `table()` again.** -->
...
**What's the distribution of coverage of the samples (`coverage` column)? Compute the `summary()` function on the coverage column. Finally, use the `hist()` function
...
`summary()` computes various useful summary statistics, and also
reports on the number of `NA` values which are missing:
...
mostly indexing (or subsetting). It's time to learn
...
**Check visually by typing out those two data frames into your console,
or using the `ncol()` on them, that you indeed have a
...
The function `contains()` unfortunately doesn't work because of the `coverage`
column:
...
function `filter()` like this (**please try it yourself!**):
...
**Whenever you will pipe something like this in your solutions, always get
...
(**Just take a moment to read both of these versions and compare them
to convince youself they do the same.**)
...
Germany and have coverage higher than 3 _or_ individuals who are from Estonia
with coverage less or equal to 1. Save your result to `df_subset` variable
...
table again (just one row for brevity). Observe the columns of the resulting
table:
...
Do not save the result back to the `df` variable though! Just run the
`select()` command on its own.**
...
3. `rename()` function to give some columns a shorter name:
...
accepts the `new_name = old_name` renaming pattern used by the `rename()` function, which
allows you to both select columns (and rename some of them) all at once.
To practice this, create the `df_subset` data frame again, but this time using just `select()`.**
...
to keep the entire data set intact (with just columns rearranged).**

**Please make sure to save the result to `df` again, so that
the metadata table has renamed columns from now on (it will
be important later). Just assign the `select()` result back
into `df` using the `<-` assignment operator.**
...
using R for ten years professionally. But there's a lot of code out
there using `select()` for this, so it's good to be aware of this
technique.
...
individuals with the condition `set != "Archaic"` and then pipe `%>%` the result
into `arrange()` for sorting based on the column `age`.
...
**Please note that you can also modify an existing column using the same
command. In this case, `<new column name>` above could simply be the name
...
(because we'll soon add them back again ourselves):**
...
we could do this, starting with a simple example of assignment of sex description
...
will contain a character vector of either "male" (if `sex == "XY"`) or else assigns "female".**
...
**Run `table(df$sex, df$sex_desc)` to make sure the assignment of sex
...
Take a look at your `mutate()` and `if_else()` code again to make sure you see why we did it wrong.
...
(**try running this in your R console to see this in practice!**):
...
**<u>Bonus exercise (feel free to skip this):</u>** How would you compute 95% confidence interval for the mean coverage using the
