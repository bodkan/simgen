...
df <- read_tsv("files/tidy/metadata.tsv")
```
...
**Wheneevr you will pipe something like this in your solutions, always get
in the habit of using these shortcuts. Eventually this will allow you to
write code as quickly as you can think, trust me!**
...
2. **Then pipe the result of the `filter()`-ing above to `select()` columns: `sampleId`, `site`, `sex`, and `hgMT` and `hgYMajor`
...
to solve the exercise in this section.**
...
**Select individuals who are from
...
**Why do we need to `print()` the result like this? What happens when you
just type `df_subset` into your R console (or executed it from your
...
After discussing `select()`, `filter()`, and `relocate()`, let's introduce
...
**Experiment with the function `relocate()`. Try it with our `df` table and
...
instead of `relocate()`), and when you add `everything()` after the last column
whose name you specify, inside the `select()` function call?**
...
`if_else()`. For instance, create a different logical vector (with an arbitrary
...
figured it out. :P Reach out to me if you want to discuss the solution!
...
## Exercise 14: Some more exploratory practice questions about the data
