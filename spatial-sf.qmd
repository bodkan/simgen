...
outlines of the continents of the world. You can do this using `geom_sf()`
...
the `sf_metadata_eur` by Y chromosome haplogroup of a sample?**
...
effectively turns the IBD table into a spatial table too, because after this
...
(longitude and latitude of `sample2`, and the same for `sample2`).
...
```{r}
# filter for IBDs with one individual being from present-day Estonia
...
```{r}
# filter for IBDs with one individual being from present-day Ukraine
...
[this study](https://english.elpais.com/elpais/2018/10/03/inenglish/1538568010_930565.html).

```{r}
# filter for IBDs with one individual being from present-day Spain
...
```{r}
# filter for IBDs with one individual being from present-day Denmark
