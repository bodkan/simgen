...
Unlike other finished chapters which feature extensive explanations and a proper
problem-solution set of exercises, **the material for this session is still
...
requirement to being able to do modern spatial data analysis.
...
**Read other data science packages:**
...
informative as it should be. But, in any case, here's how you fix it: run
...
you type `sf_metadata` to your R console? Compare this to the output you got
...
[Wikipedia article about WGS 84](https://en.wikipedia.org/wiki/EPSG_Geodetic_Parameter_Dataset) if you're
...
visualizations at the end), discussing spatial data is much easier to do the
other way around, starting from visualizations instead.
...
geom function called `geom_sf()`. You the following pattern, however, in which
you specify everything not in the `ggplot()` function, but in `geom_sf()`:**
...
If you squint hard enough, you might see an outline of the world "painted" using
...
But we want to add the outline of the World continent, to make the figure look 
nicer.**
...
spatial figures work exactly the same as _ggplot2_ figures.
...
**Notice that the legend for each color of our figure shows a huge number of
...
```{r}
#| eval: false
geom_sf(data = filter(sf_metadata, country == "Europe"), aes(color = country))
```
...
to create a new (maybe just temporary) variable. This can be useful if you
...
function call. This does the same thing as the code just above:
...
the `sf_metadata_eur` by Y chromosome haplogroup of a sample (where applicable)?**
...
**Obviously, we have data from many female individuals with (obviously) missing
...
information about the ages of our individuals:
...
**I'll leave historical and evolutionary interpretation to you, at this stage.** 🙂
...
This functions downloads all sorts of spatial data made available
by the public domans [Natural Earth](https://www.naturalearthdata.com)
project.
...
We will first download the aggregate IBD data, and do a little table-munging:

```{r}
ibd_sum <-
  read_tsv("http://tinyurl.com/simgen-ibd-sum") %>%
  filter(distance > 0) %>%
  select(sample1, sample2, total_ibd)
```
...
**Join the IBD data table with the (spatial!) table of metadata. This
effectively also turns the IBD table into a spatial table, because it
now also has spatial geometry columns for both `sample1` and also `sample2`:**

```{r}
sf_ibd <-
  # take the aggregate IBD table
  ibd_sum %>%
  # join in spatial location and metadata of the Estonian sample1
  ...
  # join in spatial location and metadata of the "source" second individual
...
sf_ibd <- relocate(sf_ibd, sample1, sample2, total_ibd, link, country1, country2, time_bin1, time_bin2)
st_crs(sf_ibd) <- st_crs(sf_metadata)
```

```{r}
sf_ibd_target <- filter(sf_ibd, country1 == "Estonia")
ggplot() +
  geom_sf(data = sf_countries) +
  geom_sf(data = sf_ibd_target, aes(color = country2)) +
  coord_sf(xlim = c(-25, 70), ylim = c(30, 80)) +
  guides(fill = "none") +
  theme_bw() +
