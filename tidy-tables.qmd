...
Before we begin, let's introduce the a couple of crucial parts of the _tidyverse_
...
2. [_tidyr_](https://tidyr.tidyverse.org) helps us morph untidy data into tidy data frames;

3. [_readr_](https://readr.tidyverse.org) provides very functions for reading
...
([reference](https://www.nature.com/articles/s41586-023-06865-0)). Feel free
to skim the paper to get an idea. If you work with anything related to human
history, you'll become closely familiar with this study.

**We use the `read_tsv()` function (from the above-mentioned _readr_ package)
and store it in a variable `df`.** Everything in these exercises will
...
<!-- ```{r} -->
<!-- # remove unnecessary columns from the original metadata -->
<!-- # df <- df %>% -->
<!-- #   select(-c(shape, starts_with("cluster"), color, callset, colorA, shapeA, flag)) %>% -->
<!-- #   write_tsv("files/tidy/metadata.tsv") -->
<!-- ``` -->
...
chapter of the _R for Data Science_ text book.** I keep this book at hand every
time I work with R.
...
sheet downloaded from supplementary materials of a paper (our situation in
this session!), you need to familiarize
yourself with it before you do anything else.
...
**Note:** Use this list as a cheatsheet of sorts! Also, **don't forget to use the
...
1. **Inspect the `df` metadata table using the functions above, and try to
answer the following questions using the functions from the list above
(you should decide which will be appropriate for which question). Before you use one of these functions for the first time, take a moment to
...
**What variables (i.e., columns) do we have in our data?**
...
which have "age" in their name)? Which one would you use to get information
about the ages of most individuals and why? What is the distribution of dates
of aDNA individuals?**
...
**How many ancient individuals do we have? How many "modern" (i.e., present-day
humans) individuals?**
...
and which publications they come frome (`dataSource`)?**
...
from? (We'll talk about spatial data science later in detail, so let's
...
**What is the source of the sequences in our data set? Does it look like they
...
**What's the distribution of coverage of the samples? Compute the `mean()`
...
you read a new table into R. It might not always be coverage, but you will have
to do this to familiarize yourself with the data and understand its potential
issues! **Always keep an eye on strange outliers, missing data `NA`, anything
...
**Who is the oldest individuals in our data set? Who has the highest
...
**Practice `select()` by creating three new data frame objects (remember
that you can assign a value to a variable using the `<-` operator):**
...
smaller number of columns in your new data frames.**
...
`ignore.case = TRUE` set by default!). Then create the `df_ages` table again,
just like earlier, with the same criteria, but use the
three helper functions listed above.**
...
For instance, in order to find out which individuals in the metadata are archaics,
...
in the previous chapter on logical expressions. For example, we could do this:
...
**Here is the most crucial thing for you to remember! Notice that we refer
to columns of our data frame `df` (like the columns `groupAge` or `ageAverage`
right above) as if they were normal variables! In any _tidyverse_ function
we don't write `"groupAge"`, but simply `groupAge`!** When you start, you'll
probably make this kind of mistake quite often. So this is a reminder to
...
1. First filter individuals for:**
...
2. **Then select columns: `sampleId`, `site`, `sex`, and `hgMT` and `hgYMajor`
(mt and Y haplogoups)**.
...
1. First `select()` the columns `sampleId`, `popId`, `country`, `groupAge`,
`ageAverage`, and `coverage`.

2. Then use the `rename()` function to give them a shorter name:
`sampleId` -> `sample`, `popId` -> `population`, `groupAge` -> `set`, `ageAverage` -> `age`. Leave the `country` and `coverage` columns as they are.**
...
## Exercise 9: Sorting rows based on columns

When you need to sort the rows of a data frame based on a value of one or
multiple columns, you can use the function `arrange()`. Again, in the spirit
...
(Remember that you can `filter()` out rows corresponding to archaic individuals
with the condition `set != "Ancient"` and then pipe the result into `arrange()`
for sorting based on the renamed column `age`).**
...
not? Think about the amount of work these functions have to do.**
...
`arrange()`, it follows a consistent _tidyverse_pattern:
...
Don't worry, we'll be adding them back soon using our custom code.
...
Notice that we get 1 for each `TRUE` and -1 for each `FALSE`.

---

**This can be a bit confusing, so spend a bit of time playing around with
`if_else()`. For instance, create a different logical vector, and have
the function produce values "hello" and "bye" depending on `TRUE` / `FALSE`
state of each element. Just get familiar with this vectorized thinking
...
This function is extremely useful whenever we need to generate values of a new
column based on values of another column(s). The general pattern for using
it inside a `mutate()` call is this:

```
df %>%
  mutate(<new_column> = if_else(<logical vector>, true_value, false_value))
```
...
We would've noticed this in our exploratory phased, had we checked the
range of possible values of the `sex` column like this. This is why getting
familiar with all data is so crucial the first time we look at it.
...
How would you fix the `mutate()` command to work correctly?**
...
Always remember to check assumptions in your data, even the ones you consider
trivial. Functions such as `table()` and `unique()` are extremely useful for this.
...
1. Rows where `is.na(age)` is `TRUE` are set to 0, and
2. Rows where `is.na(age)` is `FALSE` are set to `age` (because those values
...
"clean up" operations like this one.
...
just two. Recall our original column `set`, which had values either "Archaic",
"Ancient", or "Modern".

For this purpose, `case_when()` is perfect. It works in a very similar
manner to `if_else()`. Consider this vector of numbers between 1 and 30:
...
logical expressions on `TRUE` / `FALSE` vectors? Now it's all paying off!
...
add the `sex_desc` column again, this time using `case_when()` and implementing
...
**I provided the logical conditions for you, you just have to put them
...
**Let's ramp up the difficulty a bit more.
[Don't panic!](https://en.wikipedia.org/wiki/Phrases_from_The_Hitchhiker%27s_Guide_to_the_Galaxy#Don't_Panic) I promise things in _tidyverse_ aren't going to
get more complicated after this, as far as your day-to-day data science work
will be concerned.**

**First, let's consider the `region` column which contains the following
values:**
...
**Here are these vectors encoded as lists, one element of the list corresponding
to a continent. Copy this in your code (I premade this for you to save you
some typing):**
...
**Use your knowledge of the `%in%` operator (introduced in the R bootcamp
exercise on indexing using logical vectors) and the fact that you can
get an element of a named list such as `continents$Africa`, to create
a new column called `continent` using the `case_when()` function.**

**Hint:** As a reminder, this is how you can use the `%in%` operator to test
whether any given element of a vector is a member of another vector (which
can be used as a condition in `case_when()`, remember!). So, the general
pattern of `case_when()` that you would use inside a `mutate()` call would
be this:

```
case_when(
  region %in% continents$"<a continent name>") ~ "<a continent name>",
  ...
)
```
...
As a reminder, this is how we can get a `TRUE` / `FALSE` vector of whether
a given element of the `region` column matches one of the continents in our
named list (let's just look at five results to avoid listing thousands of items):

```{r}
head(df$region %in% continents$Africa, 5)
```

We know that each element of `case_when()` expects a `TRUE` / `FALSE` vector.
This is then how we can use the logical vector produced using the `%in%`
operator in a `case_when()` function:
...
**Use the `table()` function on the `region` and `continent` columns of `df` to
make sure we didn't miss an item as we did with the `sex_desc` above.**

**Note:** Always do these kinds of little sanity checks when doing transformations
of your data. A little bug can always creep in and really break your analysis.

::: {.callout-note collapse="true" icon=false}
#### Click to see the solution

No region has a zero in all continents, so each region is assigned to a continent:

```{r}
table(df$region, df$continent)
```
...
columns (`select()`, `rename()`), and both (`mutate()`). We have one remaining
piece of the puzzle and that is operating on _groups of rows_. This takes
...
**Take a look at the first couple of rows of our data again:**
...
`summarize()` functions. This function works a little bit like `mutate()`,
...
rows, one row for each combination of `continent` and `set`.
...
different things at the same time, for each group! The next exercise is an
example of that.

**As you know, the mean (such as the `mean_coverage` we computed right above)
is computed as a sum of a set of values divided
by their count. Use the `group_by()` and `summarize()` code like we used above,
...
**What is the lowest coverage individual in each group? While you're doing
this, `filter()` out the `set` of "Modern" individuals because they are not
meaningful anyway (they have `NA` coverages). At the end of your `%>%`
pipeline, pipe your results into `relocate(continent, set)` to make everything
even tidier. Look up `?relocate` to find out what this function does, if you're
not sure from the output.**
...
**Who is the oldest individual we have available on each continent?**
...
Now that we have our results stored in `df_summary`, let's try writing
them to disk using `write_tsv()`. The `.tsv` stands for "tab-separated values"
...
## Exercise 14: Some questions about the data
...
**Is there evidence of a potential bias in terms of how many samples
do we have from a country and the geographical location of that country, such
as the average longitude or latitude of samples from there?**

**Hint:** Count the `n` count of samples from each country using `group_by()`
and `sumarize()` again, like you did above, and compute also the `avg_lat`
and `avg_lon` (average latitude and longitude) of samples in each country.**

**Hint:** Look up the linear model function `?lm` or perhaps
...
variable (such as average longitude or latitude of a country).
...
```{r}
lm_res <- lm(n ~ avg_lon, data = geo_df)
summary(lm_res)

plot(geo_df$avg_lon, geo_df$n)
abline(lm_res)
```
...
```{r}
lm_res <- lm(n ~ avg_lat, data = geo_df)
summary(lm_res)

plot(geo_df$avg_lat, geo_df$n)
abline(lm_res)
```
...
## Exercise 15: _tidyverse_-aware functions
