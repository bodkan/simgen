...
`popId` -> `population`, `groupAge` -> `set`, `ageAverage` -> `age`. Leave
...
**Similarly, who is the youngest ancient individual in your data?**
...
**You can see that the column `sex` contains chromosome sex determination of
each individual as either `XY` or `XX`. Create a new column `sex_desc` which
...
We could've noticed this in our exploratory phased, had we checked the
range of possible values of the `sex` column like this:
...
There are a number of options. This is simplest, we can just flipe the condition:
...
**The `region` column contains the following values:**
...
to a continent. Copy this in your code:**
...
**Hint:** As a remined, this is how you can use the `%in%` operator to test
...
```
case_when(
  region %in% continents$"<continent name>") ~ "<continent name>",
  ...
)
```
...
This is then how we can use this in a `case_when()` function:
...
Why is grouping useful? The most simple use case is computing summary statistics
on the data on a "per group" basis using the `summarize()` functions. This
function works a little bit like `mutate()`. For instance, we could compute
the mean coverage of each individual in a group like this, and arrange the
groups by the coverage available:
...
we have the new column with our summary statistic.
...
**As you know, the mean is computed as a sum of a set of values divided
by their count. Use the `group_by()` and `summarize()` code we used above,
