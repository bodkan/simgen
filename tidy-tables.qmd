...
`%>%` piping operator to perform the following transformation chain (`filter`-ing
and `select`-ing criteria):**
...
**Be as silly as you can and want to by splitting a potentially very long
`filter()` command into smaller individual `filter()` function calls, each doing a
single thing (this is how interactive _tidyverse_ workflow often looks in practice,
adding things one step at a time before we arrive at our answer).**

**Hint:** What I mean by this is that you can do this, which produces the
exact same result by taking this command:
...
## Exercise 7: Reorganizing columns

## Exercise 8: Sorting tables

## Exercise 9: Mutating tables
