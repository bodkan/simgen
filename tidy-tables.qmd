...
with the condition `group != "Ancient"` and then pipe the result into `arrange()`
...
```{r}
df %>% filter(group != "Archaic") %>% arrange(desc(age))
```
...
```{r}
df %>% filter(group == "Ancient") %>% arrange(age)
```
...
just two. Recall our original column `group`, which had values either "Archaic",
"Ancient", or "Modern".
...
**Remember how we removed the `group` column, which originally contained
...
individuals based on the broad age groups like this:

```{r}
group_by(df, continent, group)
```
...
**What does the `NaN` value for "Modern" individuals in the output of the
