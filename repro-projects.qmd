...
earlier, programming-heavy session earlier.
...
**Depending on the level of your confidence and comfort
(or whether you have your own data to work with), in this session you will
either:**
...
interactions with an R console.** This is not how reproducible research
...
which will create some new data

---

#### Generating figures

**Create an R script called `03_visualize_metadata.R`, which will read
the table in `data/processed/metadata.tsv` using `read_tsv()` function, and
create the following figures using _ggplot2_ package and the function
`ggsave()` you've learned about earlier (and don't forget to
add necessary `library(...)` calls to load all packages at the start of your
script!). Save the figure(s) in the `figures/` directory. Whether you
save individual PDF for each figure, or put multiple figures into one PDF
is up to you.**

1. Count of samples in each `age_bin` of the data frame (`geom_bar()`).

2. A histogram of `coverage` across different `age_bin` categories, only
for individuals with `age > 0` (i.e., ancient individuals), with a
facet for each category (`geom_histogram()`, `facet_wrap()`). Adjust the
`scales = "free_y"` in the `facet_wrap()` function.
...
I'm just showing the plotting code. You should be able to wrap it in a dedicated
script which will read the data and save the figure(s) to a PDF.
